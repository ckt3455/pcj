    ...
    <div class="top">
        <ul class="ul-orderempty1">
            <li <?php if (!$status) { ?> class="on" <?php } ?>>
                <a href="<?= Url::to(['order/index']) ?>" class="con">
                    <div class="tit">全部
                    </div>
                </a>
            </li>
            <?php foreach (Order::$status_message as $k => $v) {
                if ($k > 0) { ?>
                    <li <?php if ($status == $k) { ?> class="on" <?php } ?>>
                        <a href="<?= Url::to(['order/index', 'status' => $k]) ?>" class="con">
                            <div class="tit"><?= $v ?></div>
                        </a>
                    </li>
                <?php }
        ...
        <div class="m-order-management">
            <div class="wp">
                <?php foreach ($order as $k => $v) { ?>
                    <div onclick="window.location.href='<?= Url::to(['order/detail', 'id' => $v['id']]) ?>'"
                         class="box box2">
                                    ...
                                    <?php if ($k2 == $lastKey) { ?>
                                        <div class="g-botmanagement">
                                            <div class="bot1">
                                                ...
                                                <div class="price1">实付金额：<span>￥<?= $v->money ?></span></div>
                                            </div>
                                            <?php if ($v->status == 1) { ?>
                                                <div class="bot2">
                                                    <div class="date">剩余时间：<?php $time = $v->created_at + 7200 - time();
                                                        if ($time > 0) {

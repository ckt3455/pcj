...
/**
 * This is the model class for table "{{%buyer_apply}}".
 *
 * @property int $id
 ...
 */
class BuyerApply extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['user_id', 'buyer_id', 'status', 'payment', 'created_at', 'updated_at'], 'integer'],
     ...
     */
    public function apply($data, $buyer_id)
    {
        ...
        try {
            ...
            // 1. 获取用户信息并锁定行
            $buyer= Buyer::findOne($buyer_id);
            if (!$buyer) {
                throw new \Exception('用户不存在');
            }

            // 2. 验证余额是否足够
            if ($buyer->money < $data['money']) {
            ...
            // 5. 创建提现记录
            $withdrawal = new BuyerApply();
            $withdrawal->user_id = $buyer['user_id'];
            $withdrawal->buyer_id = $buyer['id'];
            ...
            if($data['payment']==1){
                //微信提现
                if(!$buyer['wx_number'] or !$buyer['wx_name']){
                    throw new \Exception('请先填写提现信息');
                }
                $accountInfo='微信账号：'.$buyer['wx_number'].'-'.'微信账户名：'.$buyer['wx_name'];
            }else if($data['payment']==2){
                //支付宝提现
                if(!$buyer['zfb_number'] or !$buyer['zfb_name']){
                    throw new \Exception('请先填写提现信息');
                }
                $accountInfo='支付宝账号：'.$buyer['zfb_number'].'-'.'支付宝账号账户名：'.$buyer['zfb_name'];
            }elseif($data['payment']==3){
                if(!$buyer['bank_number'] or !$buyer['bank_name'] or !$buyer['bank_kh']){
                    throw new \Exception('请先填写提现信息');
                }
                $accountInfo='银行卡号：'.$buyer['bank_number'].'-'.'银行账户名：'.$buyer['bank_name'].'-'.'开户行：'.$buyer['bank_kh'];
            }else{
            ...
            $buyer->money = $buyer->money-$data['money'];

            if (!$buyer->save()) {
                throw new \Exception('余额更新失败: ' . implode(', ', $buyer->getFirstErrors()));
            }
...
//            $this->sendNotification($user, $withdrawal);
    ...
    {
        return 'GTX' . date('YmdHis') . str_pad(mt_rand(0, 9999), 4, '0', STR_PAD_LEFT);
    }
    ...
    {
        $log = new BuyerMoneyLog();
        $log->user_id = $data['user_id'];
        $log->number = $data['number'];
        $log->type = $data['type'];
        $log->content = $data['content'];
        $log->buyer_id = $data['buyer_id'];
        if (!$log->save()) {

...
class LinkPager extends Widget
{
     ...
     */
    public $options = ['class' => 'infoPagging wow fadeInDown'];
    /**
    ...
    protected function renderPageButton($label, $page, $class, $disabled, $active)
    {
        ...
        $class="f14 product_list_27";
        $href=$this->pagination->createUrl($page);

        if ($disabled) {
            Html::addCssClass($options, $this->disabledPageCssClass);

            $message= Html::tag(null, '<a href="##" class="'.$class.'">'.$label.'</a>', $options);

            $message='<div class="product_list_26">'.$message.'</div>';
            return $message;
        ...
        if ($active) {
            $message=Html::tag(null, '<a href="'.$href.'" class="'.$class.'">'.$label.'</a>', $options);
            $message='<div class="product_list_26 product_list_26_active">'.$message.'</div>';
            $href="##";
